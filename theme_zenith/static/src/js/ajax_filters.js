 ...
 */
publicWidget.registry.ZenithAjaxFilters = publicWidget.Widget.extend({
    ...
    events: {
        ...
        'click .zenith-mobile-filter-toggle': '_onMobileToggle',
        'click .zenith-shop-sidebar-overlay': '_onMobileToggle',
    },
    ...
    start() {
        ...
        this.$sidebar = this.$('.zenith-shop-sidebar');
        this.$overlay = this.$('.zenith-shop-sidebar-overlay');
    ...
    async _updateResults() {
        ...
        const skeletonHtml = `
            <div id="zenith_grid_skeleton" class="row">
                ${Array(8).fill(`
                    <div class="col-6 col-md-4 col-lg-3 mb-4">
                        <div class="zenith-skeleton rounded-3 mb-2" style="height: 250px; width: 100%;"></div>
                        <div class="zenith-skeleton skeleton-text" style="width: 70%;"></div>
                        <div class="zenith-skeleton skeleton-text-sm" style="width: 40%;"></div>
                    </div>
                `).join('')}
            </div>
        `;
        ...
        try {
            ...
            console.error("Zenith AJAX Filter Error:", error);
            window.location.href = newUrl; // Fallback
        } finally {
            $('#zenith_grid_skeleton').remove();
...
export default publicWidget.registry.ZenithAjaxFilters;
