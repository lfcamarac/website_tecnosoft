...
publicWidget.registry.ZenithThemeCustomizer = publicWidget.Widget.extend({
    selector: '#zenith_customizer',
    events: {
    ...
    start: function () {
        console.log("ZenithThemeCustomizer started");
        this.$panel = this.$el;
    ...
    _onColorChange: function (ev) {
        ...
        const styleId = 'zenith-custom-color-override';
        let $style = $('#' + styleId);
        if (!$style.length) {
    ...
    _onFontChange: function (ev) {
        ...
        localStorage.setItem('zenith_font_family', font);
    },
    ...
    _onDarkModeChange: function (ev) {
        ...
        if (isDark) {
         ...
         localStorage.setItem('zenith_dark_mode', isDark);
    },

    _onReset: function () {
        localStorage.removeItem('zenith_primary_color');
        localStorage.removeItem('zenith_font_family');
        localStorage.removeItem('zenith_dark_mode');
        window.location.reload();
    },

    _loadPreferences: function () {
        const color = localStorage.getItem('zenith_primary_color');
        if (color) {
            this.$(`.color-btn[data-color="${color}"]`).click();
        }
        
        const font = localStorage.getItem('zenith_font_family');
        if (font) {
            this.$(`.btn-font[data-font="${font}"]`).click();
            // Need to re-trigger to set style immediately
        }
        
         const isDark = localStorage.getItem('zenith_dark_mode') === 'true';
         if(isDark) {
