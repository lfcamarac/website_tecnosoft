...
publicWidget.registry.ZenithInfiniteScroll = publicWidget.Widget.extend({
    selector: '.zenith-products-grid',
    ...
    _initInfiniteScroll() {
        ...
        this.$loadMoreWrapper = $('<div class="zenith-load-more-wrapper text-center my-5 w-100"></div>');
        ...
        $(window).on('scroll.zenith_infinite', () => {
    ...
    async _loadNextPage(ev) {
        ...
        try {
            ...
            const $newProducts = $(response).find('.zenith-products-grid .oe_product');
...
export default publicWidget.registry.ZenithInfiniteScroll;
