...
publicWidget.registry.ZenithStoreLocator = publicWidget.Widget.extend({
    selector: '.s_zenith_store_locator',
    
    /**
     * @override
     */
    async start() {
        this.$mapContainer = this.$('#zenith_osm_map');
        this.$listContainer = this.$('#zenith_store_list');
    ...
    async _initLocator() {
        try {
            const data = await rpc('/theme_zenith/get_branches_locations');
    ...
    _renderMap() {
        ...
        this.map = L.map('zenith_osm_map').setView([first.lat || 10.4806, first.lng || -66.9036], 12);
...
export default publicWidget.registry.ZenithStoreLocator;
