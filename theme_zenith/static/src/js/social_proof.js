...
publicWidget.registry.ZenithSocialProof = publicWidget.Widget.extend({
    selector: '#wrapwrap',
    ...
    _fetchOrders: async function () {
        try {
            const res = await rpc('/theme_zenith/get_recent_orders');
            if (res && res.length > 0) {
                this.orders = res;
                this._startCycle();
            }
    ...
    _renderToast: function (order) {
        ...
        $('.zenith-social-proof').remove();

        const $toast = $(`
            <div class="zenith-social-proof animate__animated animate__fadeInLeft">
...
export default publicWidget.registry.ZenithSocialProof;
