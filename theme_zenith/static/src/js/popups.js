...
publicWidget.registry.ZenithPopups = publicWidget.Widget.extend({
    selector: '#wrapwrap',
    ...
    async _initExitIntentPopup() {
        ...
        const popupEl = document.getElementById('zenith_popup');
        if (!popupEl) return;
        ...
        if (window.bootstrap && window.bootstrap.Modal) {
            ...
            console.warn("Zenith: Bootstrap or Bootstrap Modal not found, skipping popup init.");
            return;
        }

        // Check if already shown in this session
        if (sessionStorage.getItem('zenith_popup_shown')) return;

        // Exit intent logic (mouse leaves top of window)
        document.addEventListener('mouseleave', (e) => {
            if (e.clientY < 0 && !sessionStorage.getItem('zenith_popup_shown')) {
        ...
        setTimeout(() => {
            if (!sessionStorage.getItem('zenith_popup_shown')) {
                this._showPopup(modal);
            }
        }, 20000);
    },
    ...
    _showPopup(modal) {
        if (modal) {
            modal.show();
            sessionStorage.setItem('zenith_popup_shown', 'true');
        }
    },
});
