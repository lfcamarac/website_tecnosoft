...
publicWidget.registry.ZenithAddressSelector = publicWidget.Widget.extend({
    ...
    async start() {
        ...
        if (this.$dropdown && this.$dropdown.length) {
            try {
                ...
                console.warn("ZenithAddressSelector: Could not fetch addresses", e);
            }
        }
        // Safe super call
    ...
    async _fetchAddresses() {
        try {
            const data = await rpc('/theme_zenith/get_user_addresses', {});
    ...
    async _onAddressSelect(ev) {
        ...
        try {
            await rpc('/theme_zenith/set_delivery_address', {
                address_id: addrId
            });
