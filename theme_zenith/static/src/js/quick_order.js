...
publicWidget.registry.ZenithQuickOrder = publicWidget.Widget.extend({
    selector: '.zenith-quick-order-page',
    events: {
    ...
    async _onBulkAddToCart(ev) {
        ...
        try {
            const res = await rpc('/theme_zenith/bulk_add_cart', { products: products });
            if (res.success) {
    ...
    async _processCSV(csvText) {
        ...
        try {
            const result = await rpc('/theme_zenith/get_products_by_skus', { skus: skus });
...
export default publicWidget.registry.ZenithQuickOrder;
