...
publicWidget.registry.ZenithDynamicSnippet = publicWidget.Widget.extend({
    selector: '.s_zenith_dynamic',
    ...
    async start() {
        ...
        const container = this.el.querySelector('.zenith_dynamic_container');
        if (!container) return;
        ...
        if (this.el.dataset.uiConfig) {
            try {
                ...
                console.error("Zenith: Invalid UI Config JSON", e);
            }
        }

        uiConfig.limit = uiConfig.limit || 4; // Ensure limit is set in uiConfig

        if (this.$target.hasClass('s_zenith_tabs')) {
    ...
    async _renderProducts(container, config, domain = []) {
        try {
            ...
            if (products && products.length > 0) {
                const element = renderToElement('theme_zenith.DynamicProductList', {
                    products: products,
            ...
            console.error("Zenith Snippet Error:", error);
    ...
    async _initTabs(container, config) {
        const header = this.$target[0].querySelector('.zenith_tabs_header');
        ...
        try {
            const categories = await rpc('/theme_zenith/get_categories_info', { category_ids: ids });
            if (categories.length > 0) {
                ...
                links.forEach(link => {
                    link.addEventListener('click', async (ev) => {
                        ...
                        const dynamicContainer = this.$target[0].querySelector('.zenith_dynamic_container');
                        if (dynamicContainer) {
    ...
    _initSlider(container, config) {
        const wrapper = container.querySelector('.zenith-slider-wrapper');
        if (!wrapper) return;
    ...
    _syncZenithStyle() {
        ...
        const $configurator = $('.s_zenith_configurator');
...
publicWidget.registry.ZenithHeroHotspots = publicWidget.Widget.extend({
    selector: '.s_zenith_hero',

    /**
     * @override
     */
    async start() {
        const hotspots = this.el.querySelectorAll('.zenith_hotspot');
    ...
    _initHotspot(hotspot) {
        ...
        if (window.bootstrap && window.bootstrap.Popover) {
            new window.bootstrap.Popover(hotspot, {
                ...
                html: true,
                placement: 'top',
                customClass: 'zenith-hotspot-popover',
...
export default {
    ZenithDynamicSnippet: publicWidget.registry.ZenithDynamicSnippet,
    ZenithHeroHotspots: publicWidget.registry.ZenithHeroHotspots,
};
