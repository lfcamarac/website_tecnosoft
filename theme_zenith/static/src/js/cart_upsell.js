...
publicWidget.registry.ZenithCartUpsell = publicWidget.Widget.extend({
    selector: '#wrapwrap',
    events: {
    ...
    _fetchUpsellProducts: async function (productId) {
        try {
            ...
            const result = await rpc('/theme_zenith/get_upsell_products', {
            ...
            console.error("Zenith Upsell error:", e);
        }
    },

    _openUpsellModal: function (relatedProducts) {
        // Remove existing modal if any
        $('#zenith_cart_upsell').remove();
        ...
        const modalHtml = `
        <div class="modal fade" id="zenith_cart_upsell" tabindex="-1" aria-hidden="true">
        ...
        const modalEl = document.getElementById('zenith_cart_upsell');
