...
options.registry.ZenithDynamicOptions = options.Class.extend({
    start() {
...
options.registry.ZenithTimelineOptions = options.Class.extend({
    addItem() {
        const wrapper = this.$target[0].querySelector('.timeline-wrapper');
        const items = wrapper.querySelectorAll('.zenith-timeline-item');
        if (items.length > 0) {
...
options.registry.ZenithTimelineItemOptions = options.Class.extend({
    removeItem() {
        this.$target[0].remove();
    },
});

options.registry.ZenithPricingOptions = options.Class.extend({
    addCard() {
        const container = this.$target[0].querySelector('.pricing-container');
        const cards = container.querySelectorAll('.zenith-pricing-card');
        if (cards.length > 0) {
...
options.registry.ZenithPricingCardOptions = options.Class.extend({
...
options.registry.ZenithConfiguratorOptions = options.Class.extend({
