...
if (originalCall) {
    publicWidget.Widget.prototype.call = function (service) {
        try {
            ...
            if (service === 'bus' && !res) {
                console.warn("Tecnosoft: 'bus' service not found in current context. Using stub to prevent crash.");
                return {
                    subscribe: () => { console.log("Tecnosoft: Stubbed bus.subscribe called (doing nothing)"); },
                    unsubscribe: () => {},
                    trigger: () => {},
                    on: () => {},
                    off: () => {}
            ...
            console.error("Tecnosoft: Error in legacy call bridge", e);
            if (service === 'bus') {
...
window.addEventListener('unhandledrejection', (event) => {
    ...
    if (reason && (reason.message?.includes('Bad Gateway') || reason.message?.includes('Unexpected token B'))) {
        console.error("Tecnosoft Hardening: Intercepted Bad Gateway from server. Suppressing crash to keep UI alive.");
        event.preventDefault(); // Prevent standard crash dialog if possible
    }
});

console.log("Tecnosoft Hardening: Legacy stability rails initialized.");
